...
int main(int argc, char** argv)
{
  ...
  // allocate on gcdAlloc and execute on gcdExec
  int gcdAlloc, gcdExec;
  hipGetDevice(&gcdExec);
  gcdAlloc = gcdExec;

  char *s;
  if((s = getenv("GCD_ALLOC")))
      gcdAlloc = atoi(s);

  if((s = getenv("GCD_EXEC")))
      gcdExec = atoi(s);

  // printf("Memory allocated on GCD%d\n", gcdAlloc);
  // printf("Kernels launched on GCD%d\n", gcdExec);
  hipSetDevice(gcdExec);
  hipMalloc((void**)&d_c, sizeof(real)*N);

  hipSetDevice(gcdAlloc);
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(set_array<real>), dimGrid, dimBlock, 0, 0, d_a, 2.f, N);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(set_array<real>), dimGrid, dimBlock, 0, 0, d_b, .5f, N);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(set_array<real>), dimGrid, dimBlock, 0, 0, d_c, .5f, N);

  /*  --- MAIN LOOP --- repeat test cases NTIMES times --- */
  hipSetDevice(gcdExec);
  hipDeviceEnablePeerAccess(gcdAlloc, 0);
  ...
  for (k=0; k<NTIMES; k++)
  {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(STREAM_Copy<real>), dimGrid, dimBlock, 0, 0, d_a, d_c, N);
    hipDeviceSynchronize();
    end_time = std::chrono::steady_clock::now();
    times[0][k] = std::chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();

    start_time = std::chrono::steady_clock::now();
    hipLaunchKernelGGL(HIP_KERNEL_NAME(STREAM_Scale<real>), dimGrid, dimBlock, 0, 0, d_b, d_c, scalar,  N);
    hipDeviceSynchronize();
    end_time = std::chrono::steady_clock::now();
    times[1][k] = std::chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();

    start_time = std::chrono::steady_clock::now();
    hipLaunchKernelGGL(HIP_KERNEL_NAME(STREAM_Add<real>), dimGrid, dimBlock, 0, 0, d_a, d_b, d_c,  N);
    hipDeviceSynchronize();
    end_time = std::chrono::steady_clock::now();
    times[2][k] = std::chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();

    start_time = std::chrono::steady_clock::now();
    hipLaunchKernelGGL(HIP_KERNEL_NAME(STREAM_Triad<real>), dimGrid, dimBlock, 0, 0, d_b, d_c, d_a, scalar,  N);
    hipDeviceSynchronize();
    end_time = std::chrono::steady_clock::now();
